...
Public Class FORM_IVA_DOC_COMPRA_SAE
    Implements InterfazForms
    Public nmmes As Integer
    Public nmanio As Integer
    Private varlista As Generic.List(Of V_IVA_DOC_COMPRA_SAE)
    ...
    Public Sub llenarcombos()
        UIGR_CVE_PROVEEDOR.DataSource = (From u In _datasistema.ContextoIVA.IVA_CLIE_PROV Select u Where u.FLG_CLIENTE = "N" And u.FLG_PROVEEDOR = "S" And u.CLAVE <> "").ToList
        UIGR_TIPO_DOC.DataSource = (From u In _datasistema.ContextoIVA.IVA_TIPO_DOCUMENTO Select u Where u.FLG_COMPRA = "S" And u.FLG_VENTA = "N").ToList
    ...
    Public Sub Consultar() Implements InterfazForms.Consultar
        Try
            ...
            varlista = (From u In _datasistema.ContextoIVA.V_IVA_DOC_COMPRA_SAE
...
#End Region
    ...
    Private Sub FORM_IVA_DOC_COMPRA_SAE_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Consultar()
    End Sub
    ...
    Private Sub UIBR_IMPORTAR_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles UIBR_IMPORTAR.ItemClick
        Try
            If (MetodosForm.Preguntar("¿Esta seguro de importar los documentos de compra?")) Then
                Principal.MostrarEspere(True)
                Dim msj As String = ""
                Dim nmerror As Integer = 0
                _datasistema.ContextoIVA.PR_IVA_IMPO_COMPRA(nmanio, nmmes, msj, nmerror)
                If (nmerror <> 0) Then
    ...
    Private Sub UIGRIDVIEW_RowUpdated(sender As Object, e As DevExpress.XtraGrid.Views.Base.RowObjectEventArgs) Handles UIGRIDVIEW.RowUpdated
        Try
            Dim row As V_IVA_DOC_COMPRA_SAE = e.Row
            
            Dim id As Integer = row.ID
            Dim doc As IVA_DOC_COMPRA_SAE = (From u In _datasistema.ContextoIVA.IVA_DOC_COMPRA_SAE
            ...
            doc.FE_RECEPCION = row.FE_RECEPCION
    ...
    Private Sub uibtn_unlock_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles uibtn_unlock.ItemClick
        Try
            Dim row As V_IVA_DOC_COMPRA_SAE = UIGRIDVIEW.GetFocusedRow
